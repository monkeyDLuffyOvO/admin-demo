    ...
    <Card :bordered="false" dis-hover class="ivu-mt">
      ...
      >
        ...
        <template slot-scope="{ row, index }" slot="status">
          <Tag color="default" size="large" v-show="row.status === 0">{{ row.audit_status | liveStatusFilter }}</Tag>
          <Tag color="orange" size="large" v-show="row.status === 1">{{ row.audit_status | liveStatusFilter }}</Tag>
          <Tag color="green" size="large" v-show="row.status === 2">{{ row.audit_status | liveStatusFilter }}</Tag>
          <Tag color="default" size="large" v-show="row.status === 3">{{ row.audit_status | liveStatusFilter }}</Tag>
        </template>
