...
var _utils = require("../../tools/src/utils");
...
var calcHeight = _tools.DomTools.calcHeight,
    ...
    isNodeElement = _tools.DomTools.isNodeElement;
...
var Methods = {
      ...
      if (treeOpts.transform) {
        if (process.env.NODE_ENV === 'development') {
          if (!treeOpts.rowField) {
            _tools.UtilTools.error('vxe.error.reqProp', ['table.tree-config.rowField']);
          }

          if (!treeOpts.parentField) {
            _tools.UtilTools.error('vxe.error.reqProp', ['table.tree-config.parentField']);
          }

          if (!treeOpts.children) {
            _tools.UtilTools.error('vxe.error.reqProp', ['tree-config.children']);
          }

          if (!treeOpts.mapChildren) {
            _tools.UtilTools.error('vxe.error.reqProp', ['tree-config.mapChildren']);
          }

          if (treeOpts.children === treeOpts.mapChildren) {
            _tools.UtilTools.error('vxe.error.errConflicts', ['tree-config.children', 'tree-config.mapChildren']);
          }

          fullData.forEach(function (row) {
            if (row[treeOpts.children] && row[treeOpts.children].length) {
              _tools.UtilTools.warn('vxe.error.errConflicts', ['tree-config.transform', "row.".concat(treeOpts.children)]);
            }
          });
        }
  ...
  cacheRowMap: function cacheRowMap(source) {
    ...
    var handleCache = function handleCache(row, index, items, path, parent, nodes) {
      var rowid = getRowid(_this10, row);
  ...
  loadTreeChildren: function loadTreeChildren(row, childRecords) {
    ...
    return this.createData(childRecords).then(function (rows) {
      if (keepSource) {
        var rowid = getRowid(_this11, row);
      ...
      _xeUtils.default.eachTree(rows, function (childRow, index, items, path, parent, nodes) {
        var rowid = getRowid(_this11, childRow);
        var rest = {
          row: childRow,
          rowid: rowid,
          seq: -1,
    ...
    var handleFunc = function handleFunc(column, index, items, path, parent) {
      ...
      if (process.env.NODE_ENV === 'development') {
        if (_this12.showOverflow && column.showOverflow === false) {
          _tools.UtilTools.warn('vxe.error.errConflicts', ["table.show-overflow=".concat(_this12.showOverflow), "column.show-overflow=".concat(column.showOverflow)]);
        }

        if (_this12.showHeaderOverflow && column.showHeaderOverflow === false) {
          _tools.UtilTools.warn('vxe.error.errConflicts', ["table.show-header-overflow=".concat(_this12.showHeaderOverflow), "column.show-header-overflow=".concat(column.showHeaderOverflow)]);
        }

        if (_this12.showFooterOverflow && column.showFooterOverflow === false) {
          _tools.UtilTools.warn('vxe.error.errConflicts', ["table.show-footer-overflow=".concat(_this12.showFooterOverflow), "column.show-footer-overflow=".concat(column.showFooterOverflow)]);
        }
      }
  ...
  isUpdateByRow: function isUpdateByRow(row, field) {
    ...
    if (keepSource) {
      ...
      if (treeConfig) {
        ...
        var matchObj = _xeUtils.default.findTree(tableSourceData, function (item) {
          return rowid === getRowid(_this16, item);
        }, treeOpts);
  ...
  updateAfterDataIndex: function updateAfterDataIndex() {
    ...
    if (treeConfig) {
      _xeUtils.default.eachTree(afterTreeFullData, function (row, index, items, path) {
        var rowid = getRowid(_this18, row);
  ...
  handleTargetLeaveEvent: function handleTargetLeaveEvent() {
    ...
    if (tooltipOpts.enterable) {
      this.tooltipTimeout = setTimeout(function () {
        var tooltip = _this30.$refs.tooltip;
        ...
        if (value === -1) {
          ...
          _xeUtils.default.eachTree([row], function (item) {
            if (row === item || !checkMethod || checkMethod({
              ...
              _this32.handleCheckboxReserveRow(row, value);
            }
          }, treeOpts);
        } // 如果存在父节点，更新父节点状态
      ...
      if (reserve) {
        if (value) {
          selectRows.forEach(function (row) {
            checkboxReserveRowMap[getRowid(_this33, row)] = row;
          });
        } else {
  ...
  getCheckboxReserveRecords: function getCheckboxReserveRecords(isFull) {
    ...
    if (checkboxOpts.reserve) {
      ...
      if (treeConfig) {
        _xeUtils.default.eachTree(afterFullData, function (row) {
          afterFullIdMaps[getRowid(_this34, row)] = 1;
        }, treeOpts);
      } else {
        afterFullData.forEach(function (row) {
          afterFullIdMaps[getRowid(_this34, row)] = 1;
    ...
    if (defaultSort) {
      ...
      if (defaultSort.length) {
        (sortConfig.multiple ? defaultSort : defaultSort.slice(0, 1)).forEach(function (item) {
          ...
          if (field && order) {
            var column = _this38.getColumnByField(field);
    ...
    if (sortConfs.length) {
      ...
      (multiple ? sortConfs : [sortConfs[0]]).forEach(function (confs) {
        ...
        if (_xeUtils.default.isString(field)) {
          column = _this39.getColumnByField(field);
        }
        ...
        if (column && (column.sortable || column.remoteSort)) {
          ...
          if (orders.indexOf(order) === -1) {
            order = getNextSortOrder(_this39, column);
          }
  ...
  reloadExpandContent: function reloadExpandContent(row) {
    if (process.env.NODE_ENV === 'development') {
      _tools.UtilTools.warn('vxe.error.delFunc', ['reloadExpandContent', 'reloadRowExpand']);
    } // 即将废弃
  ...
  handleAsyncRowExpand: function handleAsyncRowExpand(row) {
    ...
    return new Promise(function (resolve) {
      ...
      _this41.expandOpts.loadMethod({
        ...
        _xeUtils.default.remove(_this41.expandLazyLoadeds, function (item) {
          return item === row;
        });
  ...
  reloadTreeChilds: function reloadTreeChilds(row) {
    if (process.env.NODE_ENV === 'development') {
      _tools.UtilTools.warn('vxe.error.delFunc', ['reloadTreeChilds', 'reloadTreeExpand']);
    } // 即将废弃
  ...
  computeScrollLoad: function computeScrollLoad() {
    ...
    return this.$nextTick().then(function () {
      var sYOpts = _this49.sYOpts,
          sXOpts = _this49.sXOpts,
          scrollXLoad = _this49.scrollXLoad,
          scrollYLoad = _this49.scrollYLoad,
          scrollXStore = _this49.scrollXStore,
          scrollYStore = _this49.scrollYStore; // 计算 X 逻辑

      if (scrollXLoad) {
        var _computeVirtualX3 = computeVirtualX(_this49),
      ...
      var _computeVirtualY = computeVirtualY(_this49),
  ...
  scrollToTreeRow: function scrollToTreeRow(row) {
    ...
    if (treeConfig) {
      ...
      if (matchObj) {
        ...
        nodes.forEach(function (row, index) {
          if (index < nodes.length - 1 && !_this51.isTreeExpandByRow(row)) {
            rests.push(_this51.setTreeExpand(row, true));
          }
        });
      }
    }
    ...
    return Promise.all(rests).then(function () {
      return _tools.DomTools.rowToVisible(_this51, row);
    });
  },
    ...
    return this.$nextTick().then(function () {
      ...
      if (scope && $refs.tableBody && editRules) {
        ...
        if (_this52.hasCellRules(type, row, column)) {
          var cell = _this52.getCell(row, column);
